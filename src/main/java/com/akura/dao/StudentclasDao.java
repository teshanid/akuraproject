...
public interface StudentclasDao extends JpaRepository<Studentclas, Integer> {


    @Query("SELECT s FROM Studentclas s ORDER BY s.id DESC")
    Page<Studentclas> findAll(Pageable of);

    @Query("select s from Studentclas s where s.studentId.id =:studentd and s.clasId.id =:clasd")
    Studentclas findByStudentandClas(@Param("studentd") Integer studentd,@Param("clasd") Integer clasd);
